...
//此处bag包的地址需要自行修改
std::string bagfile = "/home/udi/01hts/LSLAM/imlsMatcherProject/src/bag/imls_icp.bag";
...
//step1:如果是第一帧就把它变成m_prevPointCloud
//step2:如果不是第一帧就把当前收到的消息变成nowPts并转换成EigenPointCLoud的格式
//step3:进行imls-icp的匹配
//step4：转化位姿
    void championLaserScanCallback(const champion_nav_msgs::ChampionNavLaserScanConstPtr& msg)
    {
        ...
        // 得到当前的Pose（这个pose是相对于上一时刻的坐标系的T^last_now，需要将其转化到world坐标系中右乘）
