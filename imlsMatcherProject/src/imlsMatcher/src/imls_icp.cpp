...
//去除非法数据
//std::isnan 用法和std::vector的erase用法
                                         ...
                                         double& height)
{
    ...
    if (nearPoints.size()!=nearNormals.size())
        return false;
    else {
        for (int i=0; i<nearPoints.size(); i++) {
            double weight = exp(-(x-nearPoints[i]).norm()*(x-nearPoints[i]).norm()/m_h*m_h);
            weightSum += weight;
            double proj;
            proj = weight*((x-nearPoints[i]).dot(nearNormals[i]));
            projSum += proj;
        }
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    mean = mean/nptCnt;
    //计算方差covariance
    //协方差阵：D(X)=E{[X-E(X)][X-E(X)]'}
    Eigen::Matrix2d covariance;
    covariance.setZero();
    for (int i=0; i<nptCnt; i++) {
        covariance = covariance + (nearPoints[i] - mean)*((nearPoints[i] - mean).transpose());
    }
    covariance = covariance/nptCnt;
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix2d> eigensolver(covariance);

    if (eigensolver.info() != Eigen::Success) abort();
    //特征值是升序排列的
    Eigen::Vector2d eigValues = eigensolver.eigenvalues();
    //第一个特征向量即为最小特征值对应的特征向量,矩阵的第一列
    Eigen::Matrix2d eigVectors = eigensolver.eigenvectors();
    normal = eigVectors.col(0);
//    cout << "The eigenvalues of covariance are:\n" << eigensolver.eigenvalues() << endl;
//    cout << "Here's a matrix whose columns are eigenvectors of covariance \n"
