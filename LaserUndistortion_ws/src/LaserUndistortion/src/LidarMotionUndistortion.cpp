...
public:
    ...
    {
        //转换到矫正需要的数据 startTime为当前帧激光点的起始时间，endTime为当前帧激光点的终止时间
        //startTime为msgs中的headerstamp
        ...
        //得到最终点的时间
        //endTime为beamNum和time_increment求出来的
    ...
    */
     ...
     *        激光雷达运动畸变去除分段函数;
     *        在此分段函数中，认为机器人是匀速运动；
     * @param frame_base_pose       标定完毕之后的基准坐标系 为第一个激光点的时候的机器人的位姿
    ...
    {
       //TODO
        tf::Quaternion start_rot, end_rot, mid_rot;
        tf::Vector3 start_t(frame_start_pose.getOrigin().getX(),frame_start_pose.getOrigin().getY(),1),
                    end_t(frame_end_pose.getOrigin().getX(),frame_end_pose.getOrigin().getY(),1), mid_t;
        start_rot = frame_start_pose.getRotation();
        end_rot = frame_end_pose.getRotation();
        for (size_t i=startIndex; i<beam_number+startIndex; i++) {
//           double s = (i-startIndex)/(beam_number-1);
           mid_rot = start_rot.slerp(end_rot,(i-startIndex)/(beam_number-1));
           mid_t = start_t.lerp(end_t,(i-startIndex)/(beam_number-1));
           tf::Transform frame_mid_pose(mid_rot,mid_t);
           double x = ranges[i]*cos(angles[i]);
           double y = ranges[i]*sin(angles[i]);
//           tf::Vector3 laser_point(x, y, 1);
           tf::Vector3 cal_point = frame_base_pose.inverse()*frame_mid_pose*tf::Vector3(x, y, 1);
           ranges[i] = sqrt(cal_point[0]*cal_point[0]+cal_point[1]*cal_point[1]);
           angles[i] = atan2(cal_point[1],cal_point[0]);
        }
    ...
    */
                           ...
                           tf::TransformListener * tf_)
    {
        ...
        // 每束激光数据的时间间隔
        double time_inc = (end_time - start_time) / (beamNumber - 1);
